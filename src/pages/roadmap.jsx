...
export default function Roadmap() {
    ...
    function videos(videos) {
        return (
            videos.map((video, index) => {
                return (
                    <div key={"video_" + index} className='flex-1 video-wrapper mt-4'>
                        <div>
                            <video width='100%' height='100%' controls poster={video.poster}>
                                {video.webm && <source src={video.webm} type="video/webm" />}
                                {video.hevc265 && <source src={video.hevc265} type="video/mp4" />}
                                {video.lowres && <source src={video.lowres} type="video/mp4" />}
                            </video>
                        </div>
                    </div>
                )
    ...
    function tweets(tweets) {
        return (
            tweets.map((tweet, index) => {
                return (
                    <a key={"tweet_" + index} className="mr-4" href={tweet.url} onClick={(e) => click(e)}>
                        <TwitterTweetEmbed
                            tweetId={tweet.id}
                        />
                    </a>
    ...
    function images(images) {
        ...
        images.map(image => {
            const url = image.url;
            const thumb = url.endsWith('.gif') ? url : url.replace(/\.[a-z]+$/i, '.thumb.webp');
                                    ...
                                    {year.items.map(item => (
                                        <li className="mb-5 ml-4 pt-4" key={key++}>
                                            ...
                                            {item.videos
                                                ...
                                                && <div className="mt-4 mb-3 text-base font-normal">{videos(item.videos)}</div>}
                                            {item.tweets
                                                && item.tweets.length > 0
                                                && <div className="mt-4 mb-3 text-base font-normal ">{tweets(item.tweets)}</div>}
                                            {item.images
                                                ...
                                                <div className='flex flex-col sm:flex-row'>
                                                    {item.links.map((link) => (
                                                        <a key={linkKey++} href={link.url}
                                                            className="text-white text-center bg-blue-600 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 mr-2 mb-2">
                                                            <p>
                                                                {link.title}
                                                            </p>
                                                        </a>
